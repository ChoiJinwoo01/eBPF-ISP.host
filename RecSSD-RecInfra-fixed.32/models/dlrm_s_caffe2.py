    ...
    def create_sequential_forward_ops(self, id_qs = None, len_qs = None, fc_q = None):
        ...
        if self.args.sls_type == 'dram':
            ...
            for i in range(len(self.ln_emb)):
                ...
                (tag_layer, tag_in, tag_out) = tag
                sum_s = tag_layer + ":::" + "sls" + str(i) + "_z"
                self.FeedBlobWrapper(sum_s, np.array([[0]]).astype(np.int32))
                self.emb_l += [core.BlobReference(sum_s)]

                self.W = np.random.uniform(low=-np.sqrt(n),
                                      high=np.sqrt(n),
                                      size=(n, m)).astype(np.int32)
                FlatTableType = (c_int * (m * n))
    ...
    if sls_type == 'base':
        for i in xrange(len(ln_emb)):
            ...
            for j in xrange(batchsize):
                embedding = np.zeros(m_spa)
                for k in xrange(embed_per_result):
                    embedidx = S_indices[i][j * embed_per_result + k]
                    libFlashRec.unvme_read_embedding.restype = POINTER(c_int)
                    read_embedding = []
                    if (lru[i].has_key(embedidx)):
                        localhits += 1
                        hits += 1
                        read_embedding = lru[i][embedidx]
                    else:
                        localmisses += 1
                        misses += 1
            ...
            for indices in split:
                ...
                if(1):
                    ...
                    for (resultidx, embedidx) in pairInd:
                        if embedidx in lru[i]:
                            localhits += 1
                            hits += 1
                            cachedEmbPair.append((resultidx, lru[i][embedidx]))
                        else:
                            localmisses += 1
                            misses += 1
                    ...
                    for k in xrange(batchsize):
                        listResult.append([float(result[k*batchsize+j]) / 1000000.0 for j in xrange(m_spa)])
                    listResult = np.array(listResult).astype(np.float32)

                    for (resultidx, embed) in cachedEmbPair:
                        listResult[resultidx] = np.add(listResult[resultidx], embed)
...
if __name__ == "__main__":
    ...
    libFlashRec.isp_stats_clear_unvme()
    ...
    if ((hits+misses) != 0):
        print("Cache hitrate: "+str(float((float(hits)/float(hits+misses))*100)))
    stop_workers = True
    libFlashRec.isp_stats_unvme()
    libFlashRec.isp_stats_clear_unvme()
    ...
    #print(histhitrate)
    print("\n---- Test end ----")
    print("Filtered hit rate_min: " + str(filteredhitrate.min()))
    print("Filtered hit rate_max: " + str(filteredhitrate.max()))
    print("---- Batch latency ----");
