...
# synthetic distribution (input data)
  def generate_synthetic_input_data(
      ...
      for i, size in enumerate(ln_emb):
          ...
          if num_indices_per_lookup_fixed:
              ...
              #)
              # Could make this work, but not enough time and no need to
              print("Error must have num_indices_per_lookup_fixed with synthetic data generation")
              sys.exit()

          per_table_num_indices.append(nbatches * sparse_group_size * mini_batch_size)
          # sparse indices to be used per embedding
          file_path = trace_file
          line_accesses, list_sd, cumm_sd = self.read_dist_from_file(
              file_path.replace("j", str(i))
          )

          # Remove a little bit of the wonkeyness from the beginning indices
          # when there isn't much stack distance to make use of entire CDF
          indices = self.trace_generate_lru(
              line_accesses, list_sd, cumm_sd, 2*per_table_num_indices[i], enable_padding
          )
          indices = indices[per_table_num_indices[i]:]

          per_table_indices.append(indices)
      ...
      for j in range(0, nbatches):
          ...
          for i, size in enumerate(ln_emb):
              ...
              for k in range(n):
                  ...
                  r = per_table_indices[i][
                          j*(mini_batch_size*sparse_group_size)+
                          (k+0)*(sparse_group_size):
                          j*(mini_batch_size*sparse_group_size)+
                          (k+1)*(sparse_group_size)]

                  # WARNING: if the distribution in the file is not consistent with
                  # embedding table dimensions, below mod guards against out of
                  # range access
                  if num_indices_per_lookup_fixed:
                      # It's more important to keep consistency with
                      # parameter requirements than remove duplicates
                      sparse_group = np.array(r).astype(np.int32)
                  else:
                  ...
                  # reset sparse_group_size in case some index duplicates were removed
                  if ((num_indices_per_lookup_fixed) and
                          (sparse_group_size != np.int32(sparse_group.size))):
                      print(
                          "WARNING: index duplicates were removed, changing "
                          + "the indices per lookup when it should be fixed "
                          + "(continuing anyway with irregular indices per lookup)"
                      )
  ...
  def generate_stack_distance(
      self,
      cumm_val, cumm_dist, max_i, i, enable_padding=False):
  ...
  def trace_generate_lru(
      ...
      for _ in range(out_trace_len):
          ...
          if sd == 0:  # new reference #
              ...
              mem_ref = np.uint64(line_ref * self.cache_line_size + mem_ref_within_line)
              i += 1
  ...
  def trace_profile(trace, enable_padding=False):
      ...
      line_accesses = []  # L
      for x in trace:
          r = np.uint64(x / self.cache_line_size)
          l = len(rstack)
          try:  # found #
              i = rstack.index(r)
...
# auxiliary read/write routines
  ...
  def read_dist_from_file(self, file_path):
      try:
