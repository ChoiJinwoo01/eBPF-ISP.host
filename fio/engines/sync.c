...
static enum fio_q_status fio_ispsyncio_queue(struct thread_data *td,
		struct io_u *io_u)
{
	struct fio_file *f = io_u->file;
	int ret;
	...
	if (io_u->ddir == DDIR_READ){
		struct nvme_passthru_cmd cmd = {    //64bit
			...
			.cdw3=0,
			.metadata=0,
			.addr       = (__u64)(uintptr_t) io_u->xfer_buf,
			.metadata_len=0,
			.data_len=io_u->xfer_buflen,
			.cdw10=(io_u->offset/4096),        //slba
			.cdw11=0,       //slba(upper part of 64bit)
			.cdw12=(io_u->xfer_buflen/4096) - 1,     //nlb
			.cdw13=0,
			.cdw14=0,
			.cdw15=0,
			.timeout_ms=0,
		...
		//ret = pread(f->fd, io_u->xfer_buf, io_u->xfer_buflen, io_u->offset);
		int err = ioctl(f->fd, NVME_IOCTL_IO_CMD, &cmd);
		if(err == 0){
...
static struct ioengine_ops ioengine_ispr = {
	.name		= "ispsync",
	.version	= FIO_IOOPS_VERSION,
	.queue		= fio_ispsyncio_queue,
	.open_file	= generic_open_file,
	.close_file	= generic_close_file,
	.get_file_size	= generic_get_file_size,
	.flags		= FIO_SYNCIO,
...
static void fio_init fio_syncio_register(void)
{
	...
	register_ioengine(&ioengine_ispr);
...
static void fio_exit fio_syncio_unregister(void)
{
	...
	unregister_ioengine(&ioengine_ispr);
